...
**Every result row says where it goes.** A page's own record carries `heading === title` and no ancestors, and rendering its trail would have repeated the line above it — so it got no second line at all. On a six-page site that is six of twenty-nine records, which made the list ragged and left the barest rows saying the least: a row reading only "Wave Docs" told a reader nothing about what it opened. Those rows carry the route now, which costs the index nothing and is the one place a path beats a title.
